...
import { Component } from '../src/types/schema';
 ...
 */
export function getAgentTools(editModes: { editPrompt: boolean; editData: boolean; editUICode: boolean }) {
  const tools: any[] = [
...
export class AgentToolHandler {
  ...
  private async editPrompt(newPrompt: string, reasoning: string) {
  ...
  private async editStructuredOutput(newSchema: string, reasoning: string) {
    // Validate JSON
    try {
  ...
  private async editUICode(newCode: string, reasoning: string) {
    // Update in memory only (not database yet)
  ...
  private async testComponent(testType: string) {
  ...
  private async getConversationTranscript(maxChars?: number) {
