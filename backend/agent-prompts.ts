...
export function buildAgentUserPrompt(params: {
  ...
  if (editModes.editPrompt) enabledModes.push('Prompt');
  if (editModes.editData) enabledModes.push('Structured Output (Data)');
  if (editModes.editUICode) enabledModes.push('UI Code');
